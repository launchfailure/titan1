...
class CorrelationRulesEngine:
    ...
    def _load_starter_rules(self):
        ...
        for rule in (
            DetectionRule(
                "TITAN-010",
                "RTF Embedded Active Content",
                "RTF contains executable or auto-linked embedded content",
                "high",
                lambda report, iocs: self._detect_rtf_active_content(report, iocs),
                ["T1204.002"],
            ),
            DetectionRule(
                "TITAN-011",
                "Excel 4.0 Macro Execution",
                "An XLM macro sheet invokes a high-risk execution function",
                "high",
                lambda report, iocs: self._detect_xlm_execution(report),
                ["T1059.005", "T1204.002"],
            ),
            DetectionRule(
                "TITAN-012",
                "MSI Embedded Executable Delivery",
                "An MSI contains an executable payload and network indicator",
                "high",
                lambda report, iocs: self._detect_format_delivery(
                    report, iocs, "msi", "msi_payload_"
                ),
                ["T1218.007", "T1204.002"],
            ),
            DetectionRule(
                "TITAN-013",
                "OneNote Embedded Executable Delivery",
                "A OneNote section contains an executable and network indicator",
                "high",
                lambda report, iocs: self._detect_format_delivery(
                    report, iocs, "onenote", "onenote_file_"
                ),
                ["T1204.002"],
            ),
        ):
    ...
    def _detect_rtf_active_content(
        ...
        names = [str(node.get("artifact_name") or "").lower() for node in nodes]
        has_object = any(name.startswith("rtf_object_") for name in names)
        has_executable = any(
            name.startswith("rtf_object_") and name.endswith(".exe") for name in names
        )
        summary = "\n".join(
        ...
        automatic = (
            '"auto_linked_object": true' in summary
            or '"object_update_requested": true' in summary
        )
        network = bool(
        ...
        return (
            has_rtf
            and has_object
            and (has_executable or automatic)
            and (network or automatic)
        )

    @staticmethod
    def _detect_xlm_execution(report: Dict[str, Any]) -> bool:
        functions = (
            "call(",
            "exec(",
            "fopen(",
        ...
        return any(
            str(node.get("artifact_name") or "").lower() == "office_xlm_macros.txt"
            and any(
                function in str(node.get("content_preview") or "").lower()
    ...
    @staticmethod
    def _detect_format_delivery(
        report: Dict[str, Any], iocs: Dict[str, Any], marker: str, artifact_prefix: str
    ) -> bool:
        nodes = report.get("nodes", [])
        recognized = any(
            marker
        ...
        executable = any(
            str(node.get("artifact_name") or "").lower().startswith(artifact_prefix)
        ...
        return recognized and executable and network
