...
<div class="cards"><div class="card"><b>Calibration cases</b><br>{calibration["case_count"]}</div><div class="card"><b>Live built-ins covered</b><br>{registry["covered_count"]}/{registry["live_builtin_count"]}</div><div class="card"><b>Analyzer adversarial coverage</b><br>{case_classes["covered_count"]}/{case_classes["required_component_count"]}</div><div class="card"><b>Recognition precision</b><br>{recognition["precision"]:.3f}</div><div class="card"><b>Extraction precision</b><br>{aggregate["precision"]:.3f}</div><div class="card"><b>Detection cases</b><br>{detection["corpus_size"]}</div><div class="card"><b>Built-in rules measured</b><br>{len(detection["per_rule"])}</div><div class="card"><b>Benchmark cases</b><br>{metrics["benchmark"]["case_count"]}</div><div class="card"><b>Fuzz seeds</b><br>{metrics["fuzzing"]["seed_count"]}</div><div class="card"><b>Plugin entries</b><br>{metrics["ecosystem"]["catalog_entries"]}</div></div>
<h2>Structural-analyzer adversarial cases</h2><table><thead><tr><th>Component</th><th>Malformed</th><th>Truncated</th></tr></thead><tbody>{case_class_rows}</tbody></table>
