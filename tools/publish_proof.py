...
<div class="cards"><div class="card"><b>Calibration cases</b><br>{calibration["case_count"]}</div><div class="card"><b>Detection cases</b><br>{detection["corpus_size"]}</div><div class="card"><b>Built-in rules measured</b><br>{len(detection["per_rule"])}</div><div class="card"><b>Aggregate precision</b><br>{aggregate["precision"]:.3f}</div><div class="card"><b>Aggregate recall</b><br>{aggregate["recall"]:.3f}</div><div class="card"><b>Benchmark cases</b><br>{metrics["benchmark"]["case_count"]}</div><div class="card"><b>Fuzz seeds</b><br>{metrics["fuzzing"]["seed_count"]}</div><div class="card"><b>Plugin entries</b><br>{metrics["ecosystem"]["catalog_entries"]}</div></div>
